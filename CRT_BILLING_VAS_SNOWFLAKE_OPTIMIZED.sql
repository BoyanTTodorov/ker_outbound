...
BASE AS (
  ...
  WHERE (SELECT APPLY_RANGE FROM RANGE_PARAMS) = FALSE
     OR (CL.HVR_CHANGE_TIME >= (SELECT START_TS FROM RANGE_PARAMS)
     AND  CL.HVR_CHANGE_TIME  < (SELECT END_TS   FROM RANGE_PARAMS))
),

/*-----------------------------------------------------------------------
UNPIVOTED_VAS: perform UNPIVOT in a wrapped subquery (no alias leakage)
-----------------------------------------------------------------------*/
UNPIVOTED_VAS AS (
  SELECT *
  FROM (
