...
/*-----------------------------------------------------------------------
BASE: resolve child/mother order + collect CL columns (incl. 25 VAS cols)
-----------------------------------------------------------------------*/
BASE AS (
  SELECT
    ...
    /* the 25 potential VAS columns to flatten later */
...
/*-----------------------------------------------------------------------
VASLINE via LATERAL FLATTEN (robust alternative to UNPIVOT)
-----------------------------------------------------------------------*/
VASLINE AS (
  SELECT
    B.P1CDPO, B.P1CACT, B.P1NANP, B.P1NPRE,
    B.CLCART, B.CLRCDE, B.CLNANP, B.CLNPRE,
    F.VALUE::STRING AS VAS_CODE,
  ...
  FROM BASE B,
       LATERAL FLATTEN(
         INPUT => ARRAY_CONSTRUCT(
           B.CLVA01, B.CLVA02, B.CLVA03, B.CLVA04, B.CLVA05,
           B.CLVA06, B.CLVA07, B.CLVA08, B.CLVA09, B.CLVA10,
           B.CLVA11, B.CLVA12, B.CLVA13, B.CLVA14, B.CLVA15,
           B.CLVA16, B.CLVA17, B.CLVA18, B.CLVA19, B.CLVA20,
           B.CLVA21, B.CLVA22, B.CLVA23, B.CLVA24, B.CLVA25
         )
       ) F
  LEFT JOIN MODELS.KERING_GLOBE.KBTVASP KBTV
    ON TRIM(SUBSTR(F.VALUE::STRING, 1, 7)) = TRIM(KBTV.VTCTVA)
