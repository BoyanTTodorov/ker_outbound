...
/*-----------------------------------------------------------------------
RANGE_PARAMS
- flip APPLY_RANGE to FALSE to return ALL rows
-----------------------------------------------------------------------*/
RANGE_PARAMS AS (
  SELECT
    TRUE AS APPLY_RANGE,
...
/*-----------------------------------------------------------------------
BASE_DATA: resolve child/mother order + collect CL columns (incl. 25 VAS cols)
-----------------------------------------------------------------------*/
BASE_DATA AS (
  SELECT
    ...
    /* 25 VAS columns (flattened later) */
...
/*-----------------------------------------------------------------------
VASLINE via LATERAL FLATTEN (no table alias on BASE_DATA referenced)
-----------------------------------------------------------------------*/
VASLINE AS (
  SELECT
  ...
  FROM (
    ...
    WHERE f.VALUE IS NOT NULL AND TRIM(SUBSTR(f.VALUE::STRING, 1, 7)) <> ''
  ) X
  LEFT JOIN MODELS.KERING_GLOBE.KBTVASP KBTV
    ON TRIM(SUBSTR(X.VAL, 1, 7)) = TRIM(KBTV.VTCTVA)
),

/*-----------------------------------------------------------------------
VASCODE aggregated per prep / VAS
-----------------------------------------------------------------------*/
VASCODE AS (
  SELECT
    P1CDPO, P1CACT, P1NANP, P1NPRE,
    ...
    SUM(QTY_VAS_LANCIATA)        AS QTY_VAS_LANCIATA,
    ANY_VALUE(PREPA_TYPE)        AS PREPA_TYPE,
    MAX(VAS_CLUSTER)             AS VAS_CLUSTER,
    MAX(CL_LASTUPDATE)           AS LASTUPDATE
  FROM VASLINE
  GROUP BY
    P1CDPO, P1CACT, P1NANP, P1NPRE, CLCART, CLRCDE, CLNANP, CLNPRE, TRIM(SUBSTR(VAS_CODE,1,7))
),
...
BILLING_VAS_BASE AS (
  ...
  WHERE NOT (PE.PET1PP = 0 OR (PE.PETSOL = '1' AND PE.PET1PP > 0))
    AND SH.SHTTKP = 1
    AND SH.SHTHMK <> 1
