CREATE OR REPLACE VIEW CRT_KER_04_STOCK AS
WITH
ge AS (
    SELECT *
...
inbound_interfaces AS (
    ...
    WHERE CHCACT = '100' AND CHCDPO = '001'
...
inv AS (
    ...
    INNER JOIN MODELS.KERING_GLOBE.KBSHIPP y
...
/* Single row: the official “stock” freshness */
obj_run AS (
    SELECT MAX(LAST_UPDATE) AS OBJECT_RUN
    FROM USER_MANAGED.KERING_GLOBE_USER_MODELS.OBJECT_RUN
    WHERE SITEID = 100
      AND "SCHEMA" = 'GG_KERI_PRD'
      AND "OBJECT" = 'STOCK'            -- <- maintain this row in your pipeline
      -- AND COALESCE("OBJECTSUBSET",'') = 'WEB_B'  -- optional if you split by subset
)
SELECT
    ...
    re.RERREC AS ASN,
    (SELECT OBJECT_RUN FROM obj_run) AS OBJECT_RUN,  -- constant across all rows
    'B' AS BUILDING,
    em.EMC1EM AS AISLE,
    CASE 
    ...
    CASE 
        ...
        ELSE 'OTH'
    END AS STORAGE_AREA,
    LPAD(ge.GENANN, 2, '0') || '/' || LPAD(ge.GENREC, 9, '0') AS REFLEX_RECEIPT_ID,
    ge.GECPRP AS OWNER,
...
INNER JOIN MODELS.KERING_GLOBE.KBCARTP   ca ON ge.GECART = ca.CASKUC
...
-- SELECT  FROM crt_KER_04_STOCK ORDER BY RANDOM() LIMIT 5;

-- SELECT *
-- FROM FACT_KER_04_STOCK
