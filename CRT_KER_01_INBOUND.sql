...
SENTINELS AS (
  SELECT '1900-01-01 00:00:01'::TIMESTAMP_NTZ AS TS0
),
...
EXPECTED_EDI AS (
  SELECT 
    ...
    I22.I22CITE AS ITEM, 
    NULL::VARCHAR AS GRADE,
    SUM(I22.I22QTEX)::NUMBER AS QTYEXP,
    0::NUMBER AS QTYCNF,
    (SELECT TS0 FROM SENTINELS) AS DAT_CNF,
    (SELECT TS0 FROM SENTINELS) AS DAT_PUT,
    (SELECT TS0 FROM SENTINELS) AS DAT_GATE,
    (SELECT TS0 FROM SENTINELS) AS DAT_DOCK_IN,
    (SELECT TS0 FROM SENTINELS) AS DAT_DOCK_OUT,
    (SELECT TS0 FROM SENTINELS) AS DAT_DEPART,
    (SELECT TS0 FROM SENTINELS) AS APPOINTMENT_DATE,
    NULL::VARCHAR AS DOCK,
    NULL::VARCHAR AS LPLATE,
    /* per-row change time for this branch */
    MAX(COALESCE(I22.HVR_CHANGE_TIME, (SELECT TS0 FROM SENTINELS))) AS LASTUPDATE
...
RECEIVED_IPG AS (
  SELECT
    ...
    VG.VGCART AS ITEM,
    VG.VGCQAL::VARCHAR AS GRADE,
    0::NUMBER AS QTYEXP,
    SUM(VG.VGQMVG)::NUMBER AS QTYCNF,

    /* DAT_CNF from RE* fields (string -> timestamp) */
    MAX(
      CASE
        WHEN RE.RETRVA = '0'
          THEN (SELECT TS0 FROM SENTINELS)
        ELSE TO_TIMESTAMP_NTZ(
          LPAD(RE.RESVAL,2,'0')||'-'||LPAD(RE.REMVAL,2,'0')||'-'||LPAD(RE.REJVAL,2,'0')||' '||
    ...
    MAX(CH.CHDAE9)::TIMESTAMP_NTZ AS DAT_PUT,

    /* YMS timings */
    MIN(YMA.MOArrivalDate)::TIMESTAMP_NTZ    AS DAT_GATE,
    MIN(YMB.MOBayDate)::TIMESTAMP_NTZ        AS DAT_DOCK_IN,
    MIN(YMBE.MODepartureDate)::TIMESTAMP_NTZ AS DAT_DOCK_OUT,
    MIN(YMD.MODepartureDate)::TIMESTAMP_NTZ  AS DAT_DEPART,
    ...
    MAX(
      CASE
        WHEN RE.RESREC = 0
          THEN (SELECT TS0 FROM SENTINELS)
        ELSE TO_TIMESTAMP_NTZ(
          LPAD(RE.RESREC,2,'0')||'-'||LPAD(RE.REMREC,2,'0')||'-'||LPAD(RE.REJREC,2,'0')||' '||
          SUBSTR(LPAD(RE.REHREC,6,'0'),1,2)||':'||SUBSTR(LPAD(RE.REHREC,6,'0'),3,2)||':'||SUBSTR(LPAD(RE.REHREC,6,'0'),5,2)
        )
      END
    ) AS APPOINTMENT_DATE,

    MIN(YMB.DOCK)::VARCHAR                 AS DOCK,
    MIN(YMA.MOLICENCEPLATE1)::VARCHAR      AS LPLATE,

    /* per-row change time for this branch */
    MAX(GREATEST(
      COALESCE(RE.HVR_CHANGE_TIME, (SELECT TS0 FROM SENTINELS)),
      COALESCE(VG.HVR_CHANGE_TIME, (SELECT TS0 FROM SENTINELS)),
      COALESCE(CH.HVR_CHANGE_TIME, (SELECT TS0 FROM SENTINELS))
    )) AS LASTUPDATE
  ...
  WHERE RE.RECACT = '100'
    ...
    AND (
      (SELECT APPLY_RANGE FROM RANGE_PARAMS) = FALSE
      OR (
           COALESCE(RE.HVR_CHANGE_TIME, (SELECT TS0 FROM SENTINELS)) >= (SELECT START_TS FROM RANGE_PARAMS)
       AND COALESCE(RE.HVR_CHANGE_TIME, (SELECT TS0 FROM SENTINELS)) <  (SELECT END_TS   FROM RANGE_PARAMS)
        )
      OR (
           COALESCE(VG.HVR_CHANGE_TIME, (SELECT TS0 FROM SENTINELS)) >= (SELECT START_TS FROM RANGE_PARAMS)
       AND COALESCE(VG.HVR_CHANGE_TIME, (SELECT TS0 FROM SENTINELS)) <  (SELECT END_TS   FROM RANGE_PARAMS)
        )
      OR (
           COALESCE(CH.HVR_CHANGE_TIME, (SELECT TS0 FROM SENTINELS)) >= (SELECT START_TS FROM RANGE_PARAMS)
       AND COALESCE(CH.HVR_CHANGE_TIME, (SELECT TS0 FROM SENTINELS)) <  (SELECT END_TS   FROM RANGE_PARAMS)
        )
    )
  GROUP BY
...
YY_ROLLUP AS (
  SELECT
    ...
    XX.SUPPORT,
    XX.ITEM,
    MAX(XX.GRADE)::VARCHAR AS GRADE,
    SUM(COALESCE(XX.QTYEXP,0))::NUMBER AS QTY_EXPECT,
    SUM(COALESCE(XX.QTYCNF,0))::NUMBER AS QTY_CONFIRM,
    MAX(XX.DAT_CNF)::TIMESTAMP_NTZ        AS DATE_CNF,
    MAX(XX.DAT_PUT)::TIMESTAMP_NTZ        AS DATE_PUT,
    MAX(XX.LASTUPDATE)::TIMESTAMP_NTZ     AS LASTUPDATE,
    MAX(XX.DAT_GATE)::TIMESTAMP_NTZ       AS DAT_GATE,
    MAX(XX.DAT_DOCK_IN)::TIMESTAMP_NTZ    AS DAT_DOCK_IN,
    MAX(XX.DAT_DOCK_OUT)::TIMESTAMP_NTZ   AS DAT_DOCK_OUT,
    MAX(XX.DAT_DEPART)::TIMESTAMP_NTZ     AS DAT_DEPART,
    MAX(XX.DOCK)::VARCHAR                 AS DOCK,
    MAX(XX.LPLATE)::VARCHAR               AS LPLATE,
    MAX(XX.APPOINTMENT_DATE)::TIMESTAMP_NTZ AS APPOINTMENT_DATE
  FROM XX_UNION XX
...
SOURCE_SELECT AS (
  SELECT 
    TO_HEX(MD5(
      'B' || '-' || COALESCE(YY.DEPOT::VARCHAR, 'NA') || '-' || COALESCE(YY.ACTIVITY::VARCHAR, 'NA') || '/' ||
      COALESCE(YY.ITEM::VARCHAR, 'NA') || '-' || COALESCE(YY.GRADE::VARCHAR, 'NA') || '-' ||
      COALESCE(YY.SUPPORT::VARCHAR, 'NA') || '-' || COALESCE(YY.INBOUNDREF::VARCHAR, 'NA')
    )) AS "PK_FACT_INBOUND",
    ...
    CASE WHEN YY.DAT_GATE        = (SELECT TS0 FROM SENTINELS) THEN NULL ELSE YY.DAT_GATE        END AS "TRUCK_GATE_ARRIVAL",
    CASE WHEN YY.DAT_DOCK_IN     = (SELECT TS0 FROM SENTINELS) THEN NULL ELSE YY.DAT_DOCK_IN     END AS "TRUCK_BAY_ARRIVAL",
    CASE WHEN YY.DAT_DOCK_OUT    = (SELECT TS0 FROM SENTINELS) THEN NULL ELSE YY.DAT_DOCK_OUT    END AS "TRUCK_BAY_DEPARTURE",
    CASE WHEN YY.DAT_DEPART      = (SELECT TS0 FROM SENTINELS) THEN NULL ELSE YY.DAT_DEPART      END AS "TRUCK_GATE_DEPARTURE",
    CASE WHEN YY.DATE_CNF        = (SELECT TS0 FROM SENTINELS) THEN NULL ELSE YY.DATE_CNF        END AS "UDM_RECEIVING",
    CASE WHEN YY.DATE_PUT        = (SELECT TS0 FROM SENTINELS) THEN NULL ELSE YY.DATE_PUT        END AS "UDM_POSITIONING",
    CASE WHEN YY.APPOINTMENT_DATE= (SELECT TS0 FROM SENTINELS) THEN NULL ELSE YY.APPOINTMENT_DATE END AS "APPOINTMENT_DATE",
...
RANGE_FILTER AS (
  SELECT s.*
  FROM SOURCE_SELECT s
  CROSS JOIN RANGE_PARAMS p
